...
#include <tesseract_qt/rendering/ogre2/common/shader_params.h>
#include <tesseract_qt/rendering/ogre2/ogre2_manager.h>
#include <tesseract_qt/rendering/ogre2/ogre2_material.h>
...
public:
  ...
  /// loaded from memory
public:
  std::shared_ptr<const tesseract_common::Image> textureData;

  /// \brief Pointer to image containing the normal map data if it was
  /// loaded from memory
public:
  std::shared_ptr<const tesseract_common::Image> normalMapData;

  /// \brief Pointer to image containing the roughness map data if it was
  /// loaded from memory
public:
  std::shared_ptr<const tesseract_common::Image> roughnessMapData;

  /// \brief Pointer to image containing the metalness map data if it was
  /// loaded from memory
public:
  std::shared_ptr<const tesseract_common::Image> metalnessMapData;

  /// \brief Pointer to image containing the emissive map data if it was
  /// loaded from memory
public:
  std::shared_ptr<const tesseract_common::Image> emissiveMapData;

  /// \brief Pointer to image containing the environment map data if it was
  /// loaded from memory
public:
  std::shared_ptr<const tesseract_common::Image> environmentMapData;

  /// \brief Pointer to image containing the light map data if it was
  /// loaded from memory
public:
  std::shared_ptr<const tesseract_common::Image> lightMapData;
...
public:
  ...
  /// Pixel Shader
public:
  Ogre::MaterialPtr ogreSolidColorMat;
...
public:
  Ogre::HighLevelGpuProgramPtr ogreSolidColorShader;

  /// \brief Returns the shader language code.
  /// \param[in] _graphicsAPI The graphic API.
  /// \return The shader language code string.
public:
  static std::string shaderLanguageCode(tesseract_gui::GraphicsAPI graphics_api)
  {
    switch (graphics_api)
    {
      case tesseract_gui::GraphicsAPI::OPENGL:
        return "glsl";
      case tesseract_gui::GraphicsAPI::VULKAN:
        return "glslvk";
      case tesseract_gui::GraphicsAPI::METAL:
        return "metal";
      default:
        return "invalid";
    }
...
//////////////////////////////////////////////////
Ogre2Material::Ogre2Material() : dataPtr(std::make_unique<Ogre2MaterialPrivate>()) {}

//////////////////////////////////////////////////
Ogre2Material::~Ogre2Material() { this->Destroy(); }
...
void Ogre2Material::Destroy()
{
  ...
  {
    this->ogreUnlitDatablock->getCreator()->destroyDatablock(this->ogreUnlitDatablock->getName());
  ...
  if (this->ogreMaterial)
  {
    ...
    Ogre::HighLevelGpuProgramManager::getSingleton().remove(this->dataPtr->ogreSolidColorShader);
    this->dataPtr->ogreSolidColorShader.reset();
  }

  Ogre::Root* root = Ogre2Manager::instance()->getRoot();
  ...
  for (size_t i = Ogre::HLMS_PBS; i < Ogre::HLMS_MAX; ++i)
  {
    ...
    if (hlms)
    {
      ...
      while (itor != end)
      {
        if (i == Ogre::HLMS_PBS)
        {
          Ogre::HlmsPbsDatablock* derivedDatablock = static_cast<Ogre::HlmsPbsDatablock*>(itor->second.datablock);
          for (size_t texUnit = 0; texUnit < Ogre::NUM_PBSM_TEXTURE_TYPES; ++texUnit)
          {
...
//////////////////////////////////////////////////
tesseract_common::Color Ogre2Material::Diffuse() const
{
  Ogre::Vector3 color = this->ogreDatablock->getDiffuse();
  return tesseract_common::Color(color.x, color.y, color.z, this->diffuse.A());
}

//////////////////////////////////////////////////
void Ogre2Material::SetDiffuse(const tesseract_common::Color& _color)
{
  BaseMaterial::SetDiffuse(_color);
  this->ogreDatablock->setDiffuse(Ogre::Vector3(_color.R(), _color.G(), _color.B()));
  this->UpdateTransparency();
}

//////////////////////////////////////////////////
tesseract_common::Color Ogre2Material::Specular() const
{
  Ogre::Vector3 color = this->ogreDatablock->getSpecular();
  return tesseract_common::Color(color.x, color.y, color.z, 1.0);
}

//////////////////////////////////////////////////
void Ogre2Material::SetSpecular(const tesseract_common::Color& _color)
{
  this->ogreDatablock->setSpecular(Ogre::Vector3(_color.R(), _color.G(), _color.B()));
}

//////////////////////////////////////////////////
tesseract_common::Color Ogre2Material::Emissive() const
{
  Ogre::Vector3 color = this->ogreDatablock->getEmissive();
  return tesseract_common::Color(color.x, color.y, color.z, 1.0);
}

//////////////////////////////////////////////////
void Ogre2Material::SetEmissive(const tesseract_common::Color& _color)
{
  this->ogreDatablock->setEmissive(Ogre::Vector3(_color.R(), _color.G(), _color.B()));
}
...
//////////////////////////////////////////////////
void Ogre2Material::SetAlphaFromTexture(bool _enabled, double _alpha, bool _twoSided)
{
...
{
  Ogre::HlmsMacroblock macroblock(*this->ogreDatablock->getMacroblock());
...
//////////////////////////////////////////////////
bool Ogre2Material::ReceiveShadows() const { return this->ogreDatablock->getReceiveShadows(); }
...
//////////////////////////////////////////////////
bool Ogre2Material::HasTexture() const { return !this->textureName.empty(); }

//////////////////////////////////////////////////
std::string Ogre2Material::Texture() const { return this->textureName; }

//////////////////////////////////////////////////
void Ogre2Material::SetTexture(const std::string& _name, const std::shared_ptr<const tesseract_common::Image>& _img)
{
  if (_name.empty())
  {
    this->ClearTexture();
...
//////////////////////////////////////////////////
std::shared_ptr<const tesseract_common::Image> Ogre2Material::TextureData() const { return this->dataPtr->textureData; }
...
//////////////////////////////////////////////////
bool Ogre2Material::HasNormalMap() const { return !this->normalMapName.empty(); }

//////////////////////////////////////////////////
std::string Ogre2Material::NormalMap() const { return this->normalMapName; }

//////////////////////////////////////////////////
std::shared_ptr<const tesseract_common::Image> Ogre2Material::NormalMapData() const
{
  return this->dataPtr->normalMapData;
}

//////////////////////////////////////////////////
void Ogre2Material::SetNormalMap(const std::string& _name, const std::shared_ptr<const tesseract_common::Image>& _img)
{
  if (_name.empty())
  {
    this->ClearNormalMap();
...
//////////////////////////////////////////////////
bool Ogre2Material::HasRoughnessMap() const { return !this->roughnessMapName.empty(); }

//////////////////////////////////////////////////
std::string Ogre2Material::RoughnessMap() const { return this->roughnessMapName; }

//////////////////////////////////////////////////
std::shared_ptr<const tesseract_common::Image> Ogre2Material::RoughnessMapData() const
{
...
//////////////////////////////////////////////////
bool Ogre2Material::HasMetalnessMap() const { return !this->metalnessMapName.empty(); }

//////////////////////////////////////////////////
std::string Ogre2Material::MetalnessMap() const { return this->metalnessMapName; }

//////////////////////////////////////////////////
std::shared_ptr<const tesseract_common::Image> Ogre2Material::MetalnessMapData() const
{
...
//////////////////////////////////////////////////
bool Ogre2Material::HasEnvironmentMap() const { return !this->environmentMapName.empty(); }

//////////////////////////////////////////////////
std::string Ogre2Material::EnvironmentMap() const { return this->environmentMapName; }

//////////////////////////////////////////////////
std::shared_ptr<const tesseract_common::Image> Ogre2Material::EnvironmentMapData() const
{
                                      ...
                                      const std::shared_ptr<const tesseract_common::Image>& _img)
{
  ...
  {
    this->SetTextureMapDataImpl(this->environmentMapName, _img, Ogre::PBSM_REFLECTION);
  }
}
...
//////////////////////////////////////////////////
bool Ogre2Material::HasEmissiveMap() const { return !this->emissiveMapName.empty(); }

//////////////////////////////////////////////////
std::string Ogre2Material::EmissiveMap() const { return this->emissiveMapName; }

//////////////////////////////////////////////////
std::shared_ptr<const tesseract_common::Image> Ogre2Material::EmissiveMapData() const
{
  return this->dataPtr->emissiveMapData;
}

//////////////////////////////////////////////////
void Ogre2Material::SetEmissiveMap(const std::string& _name, const std::shared_ptr<const tesseract_common::Image>& _img)
{
  if (_name.empty())
  {
  ...
  {
    this->SetTextureMapDataImpl(this->emissiveMapName, _img, Ogre::PBSM_EMISSIVE);
  }
}
...
//////////////////////////////////////////////////
bool Ogre2Material::HasLightMap() const { return !this->lightMapName.empty(); }

//////////////////////////////////////////////////
std::string Ogre2Material::LightMap() const { return this->lightMapName; }

//////////////////////////////////////////////////
unsigned int Ogre2Material::LightMapTexCoordSet() const { return this->lightMapUvSet; }
...
//////////////////////////////////////////////////
std::shared_ptr<const tesseract_common::Image> Ogre2Material::LightMapData() const
{
...
//////////////////////////////////////////////////
void Ogre2Material::SetRoughness(const float _roughness) { this->ogreDatablock->setRoughness(_roughness); }

//////////////////////////////////////////////////
float Ogre2Material::Roughness() const { return this->ogreDatablock->getRoughness(); }

//////////////////////////////////////////////////
void Ogre2Material::SetMetalness(const float _metalness) { this->ogreDatablock->setMetalness(_metalness); }

//////////////////////////////////////////////////
float Ogre2Material::Metalness() const { return this->ogreDatablock->getMetalness(); }

//////////////////////////////////////////////////
void Ogre2Material::PreRender() { this->UpdateShaderParams(); }

//////////////////////////////////////////////////
void Ogre2Material::UpdateShaderParams()
{
  if (this->dataPtr->vertexShaderParams && this->dataPtr->vertexShaderParams->IsDirty())
  {
    Ogre::MaterialPtr mat[2] = { this->Material(), this->dataPtr->ogreSolidColorMat };
    for (int i = 0; i < 2; ++i)
    {
  ...
  if (this->dataPtr->fragmentShaderParams && this->dataPtr->fragmentShaderParams->IsDirty())
  {
    Ogre::GpuProgramParametersSharedPtr ogreParams;
...
//////////////////////////////////////////////////
void Ogre2Material::UpdateShaderParams(std::shared_ptr<const ShaderParams> _params,
                                       Ogre::GpuProgramParametersSharedPtr _ogreParams)
{
  for (const auto& name_param : *_params)
  {
    auto* constantDef = Ogre::GpuProgramParameters::getAutoConstantDefinition(name_param.first);
    if (constantDef)
    {
    ...
    if (!_ogreParams->_findNamedConstantDefinition(name_param.first) &&
        !(Ogre2Manager::instance()->getGraphicsAPI() != GraphicsAPI::OPENGL &&
          (ShaderParam::PARAM_TEXTURE == name_param.second.Type() ||
           ShaderParam::PARAM_TEXTURE_CUBE == name_param.second.Type())))
    {
      gzwarn << "Unable to find GPU program parameter: " << name_param.first << std::endl;
      continue;
    }
    ...
    else if (ShaderParam::PARAM_FLOAT_BUFFER == name_param.second.Type())
    {
      ...
      uint32_t multiple = 1;
      _ogreParams->setNamedConstant(name_param.first, reinterpret_cast<float*>(buffer.get()), count, multiple);
    }
    else if (ShaderParam::PARAM_INT_BUFFER == name_param.second.Type())
    {
      ...
      // multiple other than 4 is currently only supported by GLSL
      uint32_t multiple = 1;
      _ogreParams->setNamedConstant(name_param.first, reinterpret_cast<int*>(buffer.get()), count, multiple);
    }
             ...
             ShaderParam::PARAM_TEXTURE_CUBE == name_param.second.Type())
    {
      ...
      if (common::isFile(value))
      {
        ...
        if (idx != std::string::npos)
        {
          ...
          if (!dirPath.empty() && !Ogre::ResourceGroupManager::getSingleton().resourceLocationExists(dirPath))
          {
      ...
      {
        gzerr << "Unrecognized texture type set for shader param: " << name_param.first << std::endl;
        continue;
      }
...
Ogre::MaterialPtr Ogre2Material::Material()
{
  if (!this->ogreMaterial)
  {
    ...
    Ogre::MaterialManager& matManager = Ogre::MaterialManager::getSingleton();
    this->ogreMaterial = matManager.create(this->name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);

    this->dataPtr->ogreSolidColorMat =
        matManager.create(this->name + "_solid", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);

    const auto graphicsApi = Ogre2Manager::instance()->getGraphicsAPI();

    this->dataPtr->ogreSolidColorShader = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(
    ...
    switch (graphicsApi)
    {
      ...
      case GraphicsAPI::VULKAN:
        ...
        {
          this->dataPtr->ogreSolidColorShader->setPrefabRootLayout(Ogre::PrefabRootLayout::Standard);
        }
        break;
...
//////////////////////////////////////////////////
Ogre::HlmsPbsDatablock* Ogre2Material::Datablock() const { return this->ogreDatablock; }

//////////////////////////////////////////////////
void Ogre2Material::SetTextureMapImpl(const std::string& _texture, Ogre::PbsTextureTypes _type)
{
  ...
  // temp workaround check if the model is a OBJ file
  {
    ...
    if (idx != std::string::npos)
    {
      std::string objFile = common::joinPaths(_texture.substr(0, idx), "meshes", "model.obj");
  ...
  if (_type == Ogre::PBSM_EMISSIVE && !this->ogreDatablock->getUseEmissiveAsLightmap())
  {
    tesseract_common::Image img(_texture);
    ...
    if (img.BPP() == 8u)
    {
      ...
      if (!tex)
      {
        gzmsg << "Grayscale emissive texture detected. Converting to RGB: " << rgbTexName << std::endl;
        ...
        for (unsigned int i = 0; i < img.Height(); ++i)
        {
          ...
          {
            // flip Y
            tesseract_common::Color c = img.Pixel(j, img.Height() - i - 1u);
        ...
        Ogre::TextureGpu* texture =
            textureMgr->createOrRetrieveTexture(rgbTexName,
  ...
  {
    bool isGrayscale = (Ogre::PixelFormatGpuUtils::getNumberOfComponents(tex->getPixelFormat()) == 1u);
    ...
    if (this->TextureAlphaEnabled() || isGrayscale)
    {
      if (tex)
      {
        ...
        if (this->TextureAlphaEnabled() && !Ogre::PixelFormatGpuUtils::hasAlpha(tex->getPixelFormat()))
        {
          this->SetAlphaFromTexture(false, this->AlphaThreshold(), this->TwoSidedEnabled());
        }
                                          ...
                                          Ogre::PbsTextureTypes _type)
{
  ...
  textureFlags |= Ogre::TextureFlags::AutomaticBatching;
  Ogre::TextureGpu* texture =
      textureMgr->createOrRetrieveTexture(_name,
                                          Ogre::GpuPageOutStrategy::Discard,
                                          textureFlags | Ogre::TextureFlags::ManualTexture,
                                          Ogre::TextureTypes::Type2D,
                                          Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME,
...
void Ogre2Material::Init()
{
  ...
  Ogre::Root* root = Ogre2Manager::instance()->getRoot();
  Ogre::HlmsManager* hlmsManager = root->getHlmsManager();
  this->ogreHlmsPbs = static_cast<Ogre::HlmsPbs*>(hlmsManager->getHlms(Ogre::HLMS_PBS));

  if (!this->ogreHlmsPbs)
  {
    gzerr << "Ogre HLMS PBS not ready. Is Ogre2 Render Engine initiallized?" << std::endl;
    return;
  }
  this->ogreDatablockId = this->Scene()->Name() + "::" + this->name;
  this->ogreDatablock = static_cast<Ogre::HlmsPbsDatablock*>(this->ogreHlmsPbs->createDatablock(
      this->ogreDatablockId, this->name, Ogre::HlmsMacroblock(), Ogre::HlmsBlendblock(), Ogre::HlmsParamVec()));
...
//////////////////////////////////////////////////
enum MaterialType Ogre2Material::Type() const { return MaterialType::MT_PBS; }
...
Ogre::HlmsUnlitDatablock* Ogre2Material::UnlitDatablock()
{
  if (!this->ogreUnlitDatablock)
  {
    ...
    Ogre::Root* root = Ogre2Manager::instance()->getRoot();
    Ogre::HlmsManager* hlmsManager = root->getHlmsManager();
    Ogre::HlmsUnlit* ogreHlmsUnlit = static_cast<Ogre::HlmsUnlit*>(hlmsManager->getHlms(Ogre::HLMS_UNLIT));
    if (!ogreHlmsUnlit)
    {
    ...
    this->ogreUnlitDatablock = static_cast<Ogre::HlmsUnlitDatablock*>(ogreHlmsUnlit->createDatablock(
        unlitName, unlitName, Ogre::HlmsMacroblock(), Ogre::HlmsBlendblock(), Ogre::HlmsParamVec()));
  }
...
//////////////////////////////////////////////////
void Ogre2Material::FillUnlitDatablock(Ogre::HlmsUnlitDatablock* _datablock) const
{
  if (!this->textureName.empty())
  {
    ...
    Ogre::TextureGpu* texture = textureMgr->createOrRetrieveTexture(
        baseName, Ogre::GpuPageOutStrategy::Discard, Ogre::TextureFlags::ManualTexture, Ogre::TextureTypes::Type2D);
...
void Ogre2Material::SetVertexShader(const std::string& _path)
{
  ...
  Ogre::ResourceGroupManager::getSingleton().addResourceLocation(_path, "FileSystem", "General", false);

  Ogre::HighLevelGpuProgramPtr vertexShader = Ogre::HighLevelGpuProgramManager::getSingletonPtr()->createProgram(
      "_gz_" + baseName,
      Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
      this->dataPtr->shaderLanguageCode(Ogre2Manager::instance()->getGraphicsAPI()),
  ...
  this->dataPtr->ogreSolidColorShader->setParameter("shader_reflection_pair_hint", vertexShader->getName());
  ...
  // after vertex shader has been created and the reflection pair set.
  {
    ...
    psParams->setNamedAutoConstant("inColor", Ogre::GpuProgramParameters::ACT_CUSTOM, 1u);
  }
  ...
  {
    gzwarn << "Material '" << this->Name() << "' could not be paired with special pixel shader '"
...
//////////////////////////////////////////////////
std::string Ogre2Material::VertexShader() const { return this->dataPtr->vertexShaderPath; }

//////////////////////////////////////////////////
std::shared_ptr<ShaderParams> Ogre2Material::VertexShaderParams() { return this->dataPtr->vertexShaderParams; }
...
void Ogre2Material::SetFragmentShader(const std::string& _path)
{
  ...
  if (Ogre2Manager::instance()->getGraphicsAPI() == GraphicsAPI::METAL)
  {
    ...
    std::string paramValue = "_gz_" + common::basename(this->dataPtr->vertexShaderPath);
...
//////////////////////////////////////////////////
std::string Ogre2Material::FragmentShader() const { return this->dataPtr->fragmentShaderPath; }

//////////////////////////////////////////////////
std::shared_ptr<ShaderParams> Ogre2Material::FragmentShaderParams() { return this->dataPtr->fragmentShaderParams; }
}  // namespace tesseract_gui
