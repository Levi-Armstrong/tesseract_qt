...
{
  Ogre::ManualObject* object = scene.createManualObject();
  object->begin("BaseWhiteNoLighting", Ogre::OperationType::OT_TRIANGLE_LIST);
  ...
  Ogre::Item* ogre_item = scene.createItem(ogre_mesh);

  OGRE_DELETE object;

  return ogre_item;

  //  Ogre::v1::ManualObject* object = new Ogre::v1::ManualObject("the one and only");
  //  object->begin("BaseWhiteNoLighting", Ogre::OperationType::OT_TRIANGLE_LIST);
  ...
  //  auto entity = entity_container.addUntrackedEntity(tesseract_gui::EntityContainer::RESOURCE_NS);
  //  std::string mesh_name = entity.unique_name + "::mesh";
  //  Ogre::v1::MeshPtr ogre_mesh = object->convertToMesh(mesh_name,
  //  Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME); ogre_mesh->buildEdgeList();

  //  Ogre::v1::Entity* ogre_entity = scene.createEntity(entity.unique_name, mesh_name);
                          ...
                          tesseract_scene_graph::Material::ConstPtr collision_material_override)
{
  ...
  Ogre::SceneNode* scene_node = scene.createSceneNode();
  scene_node->setName(entity.unique_name);
  ...
  {
    Ogre::Aabb aabb = getAABB(*scene_node, false);
    //    if (aabb.isFinite())
...
{
  Ogre::String mat_name = "tesseract::highlight::material";
  ...
  {
    mat =
        Ogre::MaterialManager::getSingleton().create(mat_name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
  ...
  auto entity = entity_container.addTrackedEntity(tesseract_gui::EntityContainer::VISUAL_NS, name);
  Ogre::SceneNode* wire_box_node = scene.createSceneNode();
  wire_box_node->setName(entity.unique_name);

  auto untracked_entity = entity_container.addUntrackedEntity(tesseract_gui::EntityContainer::RESOURCE_NS);
  Ogre::ManualObject* wire_box = scene.createManualObject();
  wire_box->setName(untracked_entity.unique_name);
  wire_box->clear();
  ...
  wire_box->begin(mat_name, Ogre::OperationType::OT_LINE_LIST);
...
Ogre::SceneNode* loadLinkGeometry(Ogre::SceneManager& scene,
                                  ...
                                  bool is_visual)
{
  Ogre::Item* ogre_item = nullptr;  // default in case nothing works.
  ...
  switch (geometry.getType())
  {
    case tesseract_geometry::GeometryType::SPHERE:
    {
      ...
      auto entity = entity_container.addUntrackedEntity(tesseract_gui::EntityContainer::RESOURCE_NS);
      ogre_item = scene.createItem(entity.unique_name, "tesseract_sphere.mesh");
    ...
    case tesseract_geometry::GeometryType::BOX:
    {
      ...
      auto entity = entity_container.addUntrackedEntity(tesseract_gui::EntityContainer::RESOURCE_NS);
      ogre_item = scene.createItem(entity.unique_name, "tesseract_cube.mesh");
      ogre_scale =
    ...
    case tesseract_geometry::GeometryType::CYLINDER:
    {
      ...
      auto entity = entity_container.addUntrackedEntity(tesseract_gui::EntityContainer::RESOURCE_NS);
      ogre_item = scene.createItem(entity.unique_name, "tesseract_cylinder.mesh");
    ...
    case tesseract_geometry::GeometryType::CONE:
    {
      ...
      auto entity = entity_container.addUntrackedEntity(tesseract_gui::EntityContainer::RESOURCE_NS);
      ogre_item = scene.createItem(entity.unique_name, "tesseract_cone.mesh");
    ...
    case tesseract_geometry::GeometryType::CAPSULE:
    {
      ...
      auto entity = entity_container.addUntrackedEntity(tesseract_gui::EntityContainer::RESOURCE_NS);
      ogre_item = scene.createItem(entity.unique_name, "tesseract_capsule.mesh");
    ...
    case tesseract_geometry::GeometryType::MESH:
    {
      ...
      //            static_cast<float>(mesh_scale.x()), static_cast<float>(mesh_scale.y()),
      //            static_cast<float>(mesh_scale.z()));
      ...
      //              "Could not convert mesh resource '%s'. It might be an empty mesh: %s", model_name.c_str(),
      //              e.what());
      //        }
      //        catch (Ogre::Exception& e)
      //        {
      //          CONSOLE_BRIDGE_logError("Could not load model '%s': %s", model_name.c_str(), e.what());
      //        }
      //      }
      //      else
      //      {
      ...
      ogre_item = createEntityForMeshData(scene, entity_container, mesh.getVertices(), mesh.getFaces());
    ...
    case tesseract_geometry::GeometryType::OCTREE:
    {
      ...
      //        data.point_cloud = createPointCloud(std::move(pointBuf[i]), entity_container, data.size,
      //        octomap.getSubType()); data.shape_type = octomap.getSubType();
  ...
  if (ogre_item)
  {
    ...
    Ogre::SceneNode* offset_node = scene.createSceneNode();
    offset_node->setName(entity.unique_name);
    //    std::vector<Ogre::Entity*>* meshes;

    offset_node->attachObject(ogre_item);
    offset_node->setScale(ogre_scale);
    offset_node->setPosition(offset_position);
    offset_node->setOrientation(offset_orientation);

    for (uint32_t i = 0; i < ogre_item->getNumSubItems(); ++i)
    {
      // Assign materials only if the submesh does not have one already
      Ogre::SubItem* sub = ogre_item->getSubItem(i);
      const std::string& material_name = sub->getMaterial()->getName();

      if (material_name == "BaseWhite" || material_name == "BaseWhiteNoLighting")
      {
        std::string cloned_name = material_name_generator.generate();
        sub->setMaterial(material->clone(cloned_name));
      }
      else
      {
        std::string cloned_name = material_name_generator.generate();
        sub->setMaterial(sub->getMaterial()->clone(cloned_name));
      }
    }
                                   ...
                                   const std::string& material_name)
{
  ...
  //    if (!Ogre::TextureGpuManager::getSingleton().resourceExists(filename))
  //    {
...
{
  //  Ogre::MaterialPtr mat = Ogre::MaterialManager::getSingleton().createOrRetrieve(material_name_generator.generate(),
  ...
  mat->setAmbient(col(0) * 0.5f, col(1) * 0.5f, col(2) * 0.5f);
...
//  float h = static_cast<float>((1.0 - std::min(std::max((z_pos - min_z) / (max_z - min_z), 0.0), 1.0)) *
//  color_factor);
...
void getAABBRecursive(Ogre::Aabb& aabb, Ogre::SceneNode& scene_node, Ogre::Matrix4 parent_pose, bool visible_only)
{
  ...
  for (int i = 0; i < scene_node.numAttachedObjects(); i++)
  {
    ...
    if (!visible_only || obj->isVisible())
    {
      ...
      if (obj->getMovableType() == Ogre::LightFactory::FACTORY_TYPE_NAME)
        box.setExtents(Ogre::Vector3(-0.5, -0.5, -0.5), Ogre::Vector3(0.5, 0.5, 0.5));
...
Ogre::Aabb getAABB(Ogre::SceneNode& scene_node, bool visible_only)
{
  Ogre::Aabb aabb;
  ...
  Ogre::Vector3 center = aabb.mCenter;
  Ogre::Vector3 max = aabb.getMaximum();
  Ogre::Vector3 min = aabb.getMinimum();

  aabb.setExtents(center + (scale * (min - center)), center + (scale * (max - center)));
  //  }
